...
The prototype inlined its own copy of small building blocks — icons, pills, buttons — across many files. Half have already been unified into a shared library: specifically the half that was *provably pixel-identical*. The remaining half **genuinely render differently** from file to file — the same icon name maps to a different glyph in different places (e.g. three distinct "chef" hats, two "wallet" shapes, different "trash"/"users"/"scan" forms), so a unified set has to pick one canonical form per name. Unifying them therefore cannot be done silently; it needs a human decision per conflict plus design sign-off (and the overlay icons additionally differ in accessibility semantics — they are marked decorative, the shared ones are not). That is exactly why the visual safety net in (a) comes first — it makes this cleanup safe to attempt.
