...
const ILLO_COLORS: Record<string, string> = {
  ...
  noodles:       '#E0C070', tortilla:     '#E8C88A', paneer:       '#F0EAD6', lentils: '#C97A3A',
};
...
function FeaturedRecipe({ recipe, onClick, onCook }: {
  recipe: Recipe; onClick: () => void; onCook: () => void;
}) {
  return (
        ...
        <div style={{ padding: 28, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', gap: 8 }}>
            ...
            <Button variant="secondary" icon="bookmark"
                    onClick={(e) => { e.stopPropagation(); fireToast('Recipe saved'); }}>Save</Button>
            <Button variant="ghost" icon="cart"
                    onClick={(e) => { e.stopPropagation(); fireToast('Missing ingredients added to grocery list'); }}>Add missing</Button>
          </div>
        </div>
      </div>
    </Card>
...
function RecipeCard({ recipe, onClick, onCook }: {
  recipe: Recipe; onClick: () => void; onCook: () => void;
}) {
  return (
    <Card pad={0} hover style={{ overflow: 'hidden', cursor: 'pointer', display: 'flex', flexDirection: 'column' }}
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', top: 10, right: 10 }}>
          <button onClick={(e) => { e.stopPropagation(); fireToast('Saved to your recipes'); }} style={{
...
function Recipes({ onOpenRecipe, onCookNow, search }: {
  onOpenRecipe: (id: string) => void; onCookNow: (id: string) => void; search: string;
}) {
    ...
    <div style={{ padding: '28px 28px 56px', display: 'grid', gap: 18 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {([
              ...
              <button key={q.l} onClick={() => setShelf(q.s)}
                style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <select value={diet} onChange={(e) => setDiet(e.target.value as DietFilter)} style={selStyle}>
          ...
          <select value={time} onChange={(e) => setTime(e.target.value as TimeFilter)} style={selStyle}>
...
const CUISINE_ORDER: Cuisine[] = ['Indian', 'Chinese', 'Mexican', 'Italian', 'Indo-Chinese',
...
function CuisineExplorer({ onOpenRecipe, onCookNow, search }: {
  onOpenRecipe: (id: string) => void; onCookNow: (id: string) => void; search: string;
}) {
...
function CuisineChip({ label, illo, active, onClick }: {
  label: string; illo: string; active: boolean; onClick: () => void;
}) {
  return (
    <button onClick={onClick} style={{
...
function CravingMode({ onOpenRecipe, onCookNow }: {
  onOpenRecipe: (id: string) => void; onCookNow: (id: string) => void;
}) {
    ...
    <div style={{ padding: '0 28px 28px', display: 'grid', gap: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Button variant="ghost" icon="upload" onClick={openInbox}>Import URL</Button>
...
function ReelsPlaceholder() {
  return (
    <div style={{ padding: '0 28px 28px' }}>
      <Card pad={48} style={{ textAlign: 'center', color: 'var(--ink-muted)' }}>
        <div style={{ marginBottom: 12, opacity: 0.6 }}><Illo name="chili" size={56} /></div>
        <div className="serif" style={{ fontSize: 20, color: 'var(--ink-2)', marginBottom: 4 }}>
  ...
  return (
    <div style={{ padding: '28px 28px 0' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        ]} value={tab} onChange={(v) => setTab(v as CookTab)} />
      </div>
