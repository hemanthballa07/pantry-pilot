// recipe-defs.js — Per-recipe scaled ingredient lists + step-by-step cook scripts.
//   Plain JS. Attaches to window.PPRecipeDefs = { RECIPE_DEF, ingrIllo }.
//   Each def: { ingredients: [{name, qty(base 2 servings), unit, illo, spice?}],
//              steps: (q, spice) => [{title, body, timer?, tip?, cue?}] }
//   `q(name)` returns a formatted, scaled quantity string for that ingredient.

window.PPRecipeDefs = (function () {

  // heat descriptor used in step copy
  const heatLine = {
  ...
  const RECIPE_DEF = {
    ...
    r1: {
      ...
      steps: (q, spice) => [
        ...
        { title: spice === "spicy" || spice === "extra" ? "Add cold rice and the Schezwan heat." : "Add cold rice and Schezwan sauce.",
