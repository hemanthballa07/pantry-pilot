...
const ICON_PATHS: Record<string, string> = {
  ...
  pantry:       'M5 3h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2z M3 9h18 M3 15h18 M9 3v18 M15 3v18',
  ...
  fridge:       'M6 2h12a1 1 0 011 1v18a1 1 0 01-1 1H6a1 1 0 01-1-1V3a1 1 0 011-1z M5 10h14 M8 6v1 M8 13v3',
  list:         'M8 6h13 M8 12h13 M8 18h13 M3 6h.01 M3 12h.01 M3 18h.01',
  snowflake:    'M12 2v20 M2 12h20 M5 5l14 14 M19 5L5 19',
  calendar:     'M5 5h14a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2z M3 11h18 M8 3v4 M16 3v4',
  trash:        'M3 6h18 M8 6V4a1 1 0 011-1h6a1 1 0 011 1v2 M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6 M10 11v6 M14 11v6',
...
function ExpiryBadge({ status, expires, size = 'md' }: {
  status: string; expires: string; size?: 'sm' | 'md';
}) {
...
interface ProgressProps {
  ...
  tone?: 'green' | 'amber' | 'tomato' | 'orange'; height?: number;
}
const PROGRESS_COLOR: Record<string, string> = {
  green:  'var(--green, #3B6E3D)',  amber:  'var(--amber, #C99325)',
  tomato: 'var(--tomato, #C73E2E)', orange: 'var(--orange, #D9722B)',
};
function Progress({ value, max = 100, tone = 'green', height = 8 }: ProgressProps) {
  ...
  return (
    <div style={{ height, borderRadius: 999, background: 'var(--bg-tint, rgba(0,0,0,.06))', overflow: 'hidden' }}>
      <div style={{ height: '100%', width: `${pct}%`, background: PROGRESS_COLOR[tone], borderRadius: 999, transition: 'width 300ms' }} />
    </div>
  );
}
...
function Ring({ value, max, size = 64, stroke = 8, color = 'var(--green)', trackColor = 'rgba(0,0,0,.06)', children }: RingProps) {
...
function SectionHead({ title, eyebrow, action }: { title: string; eyebrow: string; action?: ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
      <div>
        <div style={{ fontSize: 11, letterSpacing: 0.6, color: 'var(--ink-muted)', fontWeight: 700, textTransform: 'uppercase' }}>
          {eyebrow}
        </div>
        <div className="serif" style={{ fontSize: 17, color: 'var(--ink)', marginTop: 3 }}>{title}</div>
      </div>
      {action}
    </div>
  );
...
function IngredientTile({ item, onClick, compact }: { item: Item; onClick: () => void; compact?: boolean }) {
  return (
    <button onClick={onClick} className="lift" style={{
      ...
      <div style={{
        height: compact ? 64 : 88, background: TILE_TONE[item.status] ?? TILE_TONE.fresh,
...
function StatStrip({ label, value, sub, icon, tone }: {
  ...
  return (
    <Card pad={18}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: STAT_BG[tone], color: STAT_FG[tone],
...
function InventoryView({ onOpenItem, onAddItem, search = '' }: {
  onOpenItem: (id: string) => void; onAddItem: () => void; search?: string;
}) {
  ...
  const counts = LOC_TABS.reduce<Record<string, number>>((a, l) => {
    a[l.value] = l.value === 'all' ? mock.items.length : mock.items.filter(i => i.loc === l.value).length;
    return a;
  }, {});
...
function GridView({ items, onOpenItem, onAddItem }: {
  items: Item[]; onOpenItem: (id: string) => void; onAddItem: () => void;
}) {
  return (
  ...
  return (
    <button onClick={onClick} title={`${item.name} · ${item.expires}`} style={{
      ...
      <div style={{ position: 'absolute', top: 6, right: 6, width: 6, height: 6,
                      borderRadius: 999, background: SHELF_DOT[item.status] }} />
...
function UseFirstView({ onOpenItem, onCookNow }: {
  onOpenItem: (id: string) => void; onCookNow: (id: string) => void;
}) {
      ...
      <Card pad={22} style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr',
          ...
          <div>
            ...
            <h2 className="serif" style={{ margin: '6px 0 4px', fontSize: 26, letterSpacing: -0.4, lineHeight: 1.2 }}>
...
function ExpirySection({ title, items, onOpenItem, tone }: {
  title: string; items: Item[]; onOpenItem: (id: string) => void; tone: 'tomato' | 'amber' | 'green';
}) {
  if (!items.length) return null;
  const dot = tone === 'tomato' ? 'var(--tomato)' : tone === 'amber' ? 'var(--amber)' : 'var(--green)';
  return (
    <Card pad={22}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
...
function LowStockView({ onNav, onToast }: { onNav: (id: string) => void; onToast: (msg: string) => void }) {
  return (
    <Card pad={22}>
      <SectionHead title="Running low" eyebrow="Below threshold"
        action={<Button variant="ghost" size="sm" iconRight="cart"
                          onClick={() => onNav('shop')}>Add all to list</Button>} />
...
function LeftoversView({ onCookNow, onToast }: {
  onCookNow: (id: string) => void; onToast: (msg: string) => void;
}) {
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 14 }}>
        {items.map(l => (
          <LeftoverCard key={l.id} l={l} onCookNow={onCookNow}
...
const LEFTOVER_TONE: Record<string, string> = { amber: 'var(--amber)', tomato: 'var(--tomato)', green: 'var(--green)' };
const LEFTOVER_BG:   Record<string, string> = { amber: 'var(--amber-soft)', tomato: 'var(--tomato-soft)', green: 'var(--green-tint)' };

function LeftoverCard({ l, onFinish, onWaste, onAddToPlan, onCookNow }: {
  l: Leftover; onFinish: () => void; onWaste: () => void; onAddToPlan: () => void;
  ...
  return (
    <Card pad={0} hover style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{
        padding: '14px 18px', background: LEFTOVER_BG[l.tone],
      ...
      <div style={{ padding: '18px 18px 16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <Button size="sm" variant="primary" icon="chef"
                  onClick={() => onCookNow('r1')}>Remix into recipe</Button>
...
function AutoRestockView() {
  ...
  const toggle = (id: string) => setRules(r => r.map(x => x.id === id ? { ...x, on: !x.on } : x));
  return (
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 18 }}>
      <Card pad={22}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
          {staples.have.map((s, i) => (<Pill key={`${s}-${i}`} tone="green" size="sm">{s}</Pill>))}
        </div>
      </Card>

      <Card pad={22}>
        <SectionHead title="Recommended staples" eyebrow="Unlock more meals"
  ...
  return (
    <div style={{ padding: '28px 28px 0 28px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 18 }}>
        <StatStrip label="Total items" value={mock.items.length} sub="across 4 locations" icon="pantry" tone="green" />
        <StatStrip label="Use soon"
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        {tab === 'inventory' && (
          <div style={{ display: 'flex', gap: 8 }}>
            <Button size="sm" variant="ghost" icon="receipt" onClick={openInbox}>Scan receipt</Button>
            <Button size="sm" variant="primary" icon="plus" onClick={openAddItem}>Add item</Button>
          </div>
        )}
      </div>
