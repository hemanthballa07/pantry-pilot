...
function IngredientTile({ item, onClick }: { item: Item; compact?: boolean; onClick: () => void }) {
  ...
  const expColor = expDays <= 1 ? 'var(--tomato)' : expDays <= 3 ? 'var(--amber)' : 'var(--ink-muted)';
  return (
    <div onClick={onClick} style={{
...
function PlanSlot({ slot, onOpen, onCook, onToast }: {
  slot:    TodaySlot;
  onOpen:  () => void;
  onCook:  () => void;
    ...
    <div className="lift" onClick={onOpen} style={{
      ...
      <div style={{ padding: '14px 16px 16px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <Button size="sm" variant={isCook ? 'primary' : 'secondary'} full
                ...
                onClick={e => { e.stopPropagation(); if (isCook) { onCook(); } else { onToast(`${slotLabel} ready`); } }}
                style={{ marginTop: 10 }}>
...
function TodayPlan({ onOpenRecipe, onCookNow, onToast }: {
    ...
    <Card pad={0} style={{ overflow: 'hidden',
      ...
      <div style={{ padding: '26px 28px 20px', display: 'flex',
        ...
        <div>
          ...
          <h2 className="serif" style={{ margin: '6px 0 6px', fontSize: 32, letterSpacing: -0.6, lineHeight: 1.1 }}>
    ...
    <Card pad={22}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => onNav('pantry')} style={linkBtn}>
          Open pantry<Icon name="chevronRight" size={12}/>
        </button>
      </div>
...
function WasteRescue({ onCookNow, onOpenRecipe }: {
  onCookNow:    (_id: string) => void;
  onOpenRecipe: (_id: string) => void;
}) {
    ...
    <Card pad={22}>
      ...
      <div style={{ display: 'grid', gap: 8 }}>
        {wr.plan.map((p: WasteEntry) => (
          <div key={p.day} onClick={() => onOpenRecipe(p.recipe)}
            style={{
...
function ExpiringCard({ onOpenItem, onNav }: {
...
const PILOT_TONE_COLOR: Record<string, string> = {
  green: 'var(--green)', tomato: 'var(--tomato)', orange: 'var(--orange)', amber: 'var(--amber)',
};

const PILOT_ICON_MAP: Record<string, string> = {
  spark: 'sparkles', wallet: 'wallet', leaf: 'leaf', alert: 'alert',
};
  ...
  return (
    <Card pad={22}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {mock.kitchenWins.map(w => <WinCard key={w.id} win={w}/>)}
      </div>
    </Card>
  );
}
...
function WeekPreview({ onNav, onOpenRecipe }: {
  onNav:        (id: string)  => void;
  onOpenRecipe: (_id: string) => void;
}) {
      ...
      <SectionHead title="This week" eyebrow="Mon · Nov 23 — Sun · Nov 29"
        action={
          <button onClick={() => onNav('plan')} style={linkBtn}>
            Open plan<Icon name="chevronRight" size={12}/>
          </button>
        }/>
...
function GroceryMini({ onNav }: { onNav: (id: string) => void }) {
  ...
  return (
    <Card pad={22} hover onClick={() => onNav('shop')} style={{ cursor: 'pointer' }}>
      ...
      <Button full variant="soft" iconRight="arrowRight"
              onClick={e => { e.stopPropagation(); onNav('shop'); }}
              style={{ marginTop: 12 }}>
        Open list
      </Button>
    </Card>
...
function LeftoversMini({ onNav }: { onNav: (id: string) => void }) {
  ...
  return (
    <Card pad={22} hover onClick={() => onNav('pantry')} style={{ cursor: 'pointer' }}>
      ...
      <Button full variant="soft" iconRight="arrowRight"
              onClick={e => { e.stopPropagation(); onNav('pantry'); }}
              style={{ marginTop: 12 }}>
        Open leftovers
      </Button>
    </Card>
...
function ActivityCard() {
  return (
    <Card pad={22}>
      ...
      <div>
        {mock.activity.slice(0, 5).map((a, i) => <ActivityRow key={i} act={a}/>)}
      </div>
    </Card>
  );
}
