...
function Avatar({ initials, color = 'var(--green)', size = 28 }: {
...
function GroceryRow({ item, onToggle, showOwner }: {
  item: GroceryItem; onToggle: () => void; showOwner: boolean;
}) {
...
function CookBeforeShop({ onDismiss, onCookNow, onOpenRecipe }: {
  onDismiss: () => void; onCookNow: (id: string) => void; onOpenRecipe: (id: string) => void;
}) {
    ...
    <Card pad={0} style={{
      ...
      <div style={{
        ...
        <Button size="sm" variant="primary" iconRight="arrowRight"
                onClick={() => { if (recipes[0]) onCookNow(recipes[0].id); }}>Cook these first</Button>
      </div>
    </Card>
  );
}
...
function AutoRestockStrip({ onToast, onNav }: {
  onToast: (msg: string) => void; onNav: (id: string) => void;
}) {
  const pending = mock.autoRestockRules.filter(r => r.on && /day|Mon|Sun/.test(r.next)).slice(0, 3);
  ...
  return (
    <Card pad={16}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1 }} />
        <Button size="sm" variant="ghost" onClick={() => onNav('pantry')}>Rules</Button>
        <Button size="sm" variant="secondary"
                onClick={() => onToast('Added now to your list')}>
          Add now
        </Button>
      </div>
    </Card>
...
function StoreMode({ items, setItems, onExit, onToast }: {
  items: GroceryItem[];
  ...
  const modeTabs = [{ l: 'To buy', v: 'open' }, { l: 'All', v: 'all' }] as const;
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', padding: '20px 16px 80px',
      ...
      <div style={{
        ...
        <Button size="sm" variant="orange"
                onClick={() => { onToast('Trip complete · synced to pantry'); onExit(); }}>
          Finish trip
        </Button>
      </div>
    </div>
...
const STORE_COMPARISON = [
  { name: 'Aldi',         price: 38.20, fav: true,  miss: 0, dist: '1.4 mi' },
  { name: 'Walmart',      price: 42.80, fav: false, miss: 0, dist: '2.8 mi' },
  { name: "Trader Joe's", price: 46.10, fav: false, miss: 1, dist: '3.1 mi' },
  { name: 'Publix',       price: 49.10, fav: false, miss: 0, dist: '1.2 mi' },
];
...
export default function Shop() {
  ...
  if (mode === 'store') {
    return <StoreMode items={items} setItems={setItems} onExit={() => setMode('plan')} onToast={onToast} />;
  }
      ...
      <div style={{ display: 'grid', gap: 18, minWidth: 0 }}>
        ...
        <Card pad={22}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            ...
            <button onClick={openInbox} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1 }} />
          <Button variant="ghost" size="sm" icon="sparkles" onClick={openAskPilot}>Pilot suggest</Button>
          <Button variant="primary" size="sm" icon="plus" onClick={openAddItem}>Add item</Button>
        </div>
