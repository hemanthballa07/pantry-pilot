...
const ICON_PATHS: Record<string, string> = {
  ...
  camera:      'M4 8a2 2 0 0 1 2-2h2l1.5-2h5L16 6h2a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2z M8.5 13a3.5 3.5 0 1 0 7 0a3.5 3.5 0 1 0 -7 0',
...
interface CardProps {
  ...
  hover?: boolean; onClick?: (e: React.MouseEvent) => void;
}
...
interface TabDef { label: string; value: string; }
...
function Tabs({ tabs, value, onChange }: { tabs: TabDef[]; value: string; onChange: (v: string) => void }) {
  return (
    <div style={{ display: 'inline-flex', gap: 4, background: 'var(--bg-tint)',
                    padding: 4, borderRadius: 12, border: '1px solid var(--line)', flexWrap: 'wrap' }}>
      {tabs.map(t => {
...
function Avatar({ initials, color = 'var(--green)', size = 32 }: {
  ...
  return (
    <div style={{
      ...
      fontSize: size * 0.4, fontWeight: 700, letterSpacing: 0.3, flexShrink: 0,
    }}>{initials}</div>
  );
}

function AvatarStack({ people, size = 32 }: { people: Array<{ initials: string; color?: string }>; size?: number }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center' }}>
      {people.map((p, i) => (
        <div key={i} style={{
          marginLeft: i === 0 ? 0 : -(size * 0.3), zIndex: people.length - i,
          width: size, height: size, borderRadius: 999,
          background: p.color ?? 'var(--ink, #1a1814)', color: '#FDFAF3',
...
function SectionHead({ title, eyebrow, action }: { title: string; eyebrow: string; action?: ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 14 }}>
      <div>
        <div style={{ fontSize: 10.5, letterSpacing: 0.8, textTransform: 'uppercase',
...
function BudgetStat({ label, value, sub, tone, icon, up, down }: {
  label: string; value: string; sub: string;
  tone: 'green' | 'tomato' | 'amber' | 'neutral'; icon: string; up?: boolean; down?: boolean;
}) {
...
function Donut({ segments, size = 160 }: {
  segments: { start: number; end: number; color: string }[]; size?: number;
}) {
  const r = size / 2 - 16;
  ...
  return (
    <div style={{ padding: '28px 28px 56px', display: 'grid', gap: 18 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        <BudgetStat label="Spent this month"  value={`$${b.spent.toFixed(0)}`} sub={`of $${b.monthly} budget`} tone="green" icon="wallet" />
        <BudgetStat label="Saved from pantry" value={`$${b.savedFromPantry}`} sub="vs. takeout average" tone="green" icon="leaf" up />
        <BudgetStat label="Food waste"        value={`$${b.waste.toFixed(2)}`} sub="3 items · -28% MoM" tone="tomato" icon="trash" down />
        <BudgetStat label="Avg meal cost"     value="$1.85" sub="based on 19 meals" tone="neutral" icon="chef" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
        ...
        <Card pad={22}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, alignItems: 'end', height: 200 }}>
            {b.weekly.map((w, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                  ...
                  <div style={{
                    ...
                    background: i === 3 ? 'var(--green)' : 'linear-gradient(180deg, var(--orange) 0%, var(--orange-deep) 100%)',
            ...
            <div style={{ flex: 1, display: 'grid', gap: 6 }}>
              {b.byCategory.map(c => (
                <div key={c.cat} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span className="tnum" style={{ fontSize: 12, color: 'var(--ink-muted)' }}>${c.amount.toFixed(0)}</span>
                </div>
              ))}
            </div>
          </div>
...
function Waste() {
  ...
  mock.wasteLog.forEach(l => { byReason[l.reason] = (byReason[l.reason] || 0) + l.cost; });
                ...
                <tr style={{ background: 'var(--bg-tint)' }}>
                  {['Item', 'Reason', 'Qty', 'Cost', 'Date'].map(h => (
                    <th key={h} style={{
                      ...
                      color: 'var(--ink-muted)', letterSpacing: 0.4, textTransform: 'uppercase', fontWeight: 700,
                    }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {mock.wasteLog.map((l, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--line-soft)' }}>
                    ...
                    <td style={{ padding: '10px 14px' }}><Pill tone="ghost" size="sm">{l.reason}</Pill></td>
              ...
              {reasons.map(([r, c]) => (
                <div key={r}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span className="tnum" style={{ fontSize: 12, color: 'var(--ink-muted)' }}>${c.toFixed(2)}</span>
                  </div>
        ...
        {Array.from({ length: 12 }, (_, w) => (
          <div key={w} style={{ display: 'grid', gridTemplateRows: 'repeat(7, 1fr)', gap: 3 }}>
            {Array.from({ length: 7 }, (_, d) => (
              <div key={d} style={{ aspectRatio: '1', borderRadius: 3, background: colors[days[w * 7 + d]] }} />
            ))}
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14, fontSize: 11.5, color: 'var(--ink-muted)' }}>
        <span>Less</span>
        {colors.map((c, i) => (
...
function MemoryDeep() {
  return (
    <div style={{ padding: '0 28px 28px', display: 'grid', gap: 18 }}>
      <Card pad={26} style={{ background: 'var(--ink)', color: 'var(--paper-warm)', borderColor: 'var(--ink)' }}>
          ...
          <Card key={s.id} pad={0} hover style={{ overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px solid var(--line-soft)' }}>
              <div style={{ padding: '16px 18px', background: 'var(--tomato-soft)' }}>
                ...
                <div className="serif" style={{ fontSize: 18, color: 'var(--ink)', marginTop: 4, lineHeight: 1.2 }}>{s.takeout}</div>
              ...
              <div style={{ padding: '16px 18px', background: 'var(--green-tint)' }}>
                ...
                <div className="serif" style={{ fontSize: 18, color: 'var(--ink)', marginTop: 4, lineHeight: 1.2 }}>{s.home}</div>
            ...
            <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              ...
              <Button variant="primary" iconRight="arrowRight"
                      onClick={() => fireToast('Cook Mode opens in a later update')}>Cook the swap</Button>
            </div>
          </Card>
        ))}
      </div>
...
function NutLine({ label, v, unit, color }: {
  label: string; v: { value: number; target: number; unit?: string }; unit?: string; color: string;
}) {
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        <span className="tnum" style={{ fontSize: 12, color: 'var(--ink-muted)' }}>
          <strong style={{ color: 'var(--ink)' }}>{v.value}{u}</strong>
          <span style={{ marginLeft: 4 }}>/ {v.target}{u}</span>
        </span>
      </div>
    ...
    <div style={{ padding: '28px 28px 56px', display: 'grid', gap: 18 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 18 }}>
        <Card pad={26}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
            <div style={{ position: 'relative', width: 200, height: 200 }}>
              <Ring value={ringPct(n.cals)} size={200} stroke={16} color="var(--orange)" trackColor="var(--bg-tint)">
                <Ring value={ringPct(n.protein)} size={158} stroke={14} color="var(--tomato)" trackColor="var(--bg-tint)">
                  <Ring value={ringPct(n.fiber)} size={118} stroke={12} color="var(--green)" trackColor="var(--bg-tint)">
                    <div style={{ textAlign: 'center' }}>
                      <div className="serif tnum" style={{ fontSize: 24, color: 'var(--ink)', lineHeight: 1 }}>
                        {Math.round(ringPct(n.cals))}%
                      </div>
                      <div style={{ fontSize: 10, color: 'var(--ink-muted)', marginTop: 3, fontWeight: 600, letterSpacing: 0.4 }}>
                        OF TARGET
                      </div>
                    </div>
                  </Ring>
        ...
        <Card pad={22} style={{ background: 'var(--green-tint)', borderColor: 'var(--green-soft)' }}>
          ...
          <div className="serif" style={{ fontSize: 22, color: 'var(--ink)', letterSpacing: -0.3, lineHeight: 1.2, marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
            {[
              ...
              <div key={s.l} style={{
                ...
                <div style={{ fontSize: 10.5, color: 'var(--green-deep)', marginTop: 2 }}>{s.p} · ${s.c}</div>
              </div>
            ))}
          </div>
          <Button size="sm" variant="primary" onClick={() => fireToast('+3 eggs added to dinner')}>Apply suggestion</Button>
        </Card>
      </div>
          ...
          {[68, 84, 72, 90, 65, 76, 60].map((v, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              <div style={{ position: 'relative', width: '100%', height: 140,
                ...
                <div style={{ position: 'absolute', left: 0, right: 0, top: `${140 - (90 / 100) * 140}px`,
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {[
            ...
            <div key={m.meal} style={{
              display: 'grid', gridTemplateColumns: '80px 1fr repeat(4, 70px)', gap: 12, alignItems: 'center',
              padding: '12px 16px', borderRadius: 10,
              background: 'var(--paper-warm)', border: '1px solid var(--line-soft)',
            }}>
              <div style={{ fontSize: 11.5, color: m.color, fontWeight: 700, letterSpacing: 0.4, textTransform: 'uppercase' }}>{m.meal}</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--ink)' }}>{m.name}</div>
              {[`${m.cal} cal`, `${m.p}g pro`, `${m.c}g car`, `${m.f}g fat`].map((vv, i) => (
                <div key={i} className="tnum" style={{ fontSize: 12, color: 'var(--ink-muted)', textAlign: 'right' }}>{vv}</div>
              ))}
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ padding: '28px 28px 0' }}>
      <div style={{ marginBottom: 18 }}>
        ...
        ]} value={tab} onChange={(v) => setTab(v as InsightsTab)} />
      </div>
      ...
      <Card pad={26}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <Button variant="primary" icon="plus"
                  onClick={() => fireToast('Invite link copied to clipboard')}>Invite a roommate</Button>
        </div>
      </Card>
        ...
        {mock.household.map(p => (
          <Card key={p.id} pad={22}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              ...
              <div>
                <div className="serif tnum" style={{ fontSize: 22, color: 'var(--ink)', lineHeight: 1 }}>${p.spent.toFixed(0)}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 18 }}>
        <Card pad={22}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            ...
            <Button variant="ghost" size="sm" iconRight="chevronRight" onClick={() => navigate('/pantry')}>Open pantry</Button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {mock.items.filter(i => i.owner === 'Shared').slice(0, 8).map(it => (
              <div key={it.id} style={{
        ...
        <Card pad={22}>
          ...
          <div style={{ marginTop: 10 }}>
            {mock.activity.slice(0, 6).map((a, i) => <ActivityRow key={i} act={a} />)}
          </div>
        </Card>
      </div>
    </div>
  ...
  return (
    <Card pad={26}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <Button variant="primary" iconRight="arrowRight"
                onClick={() => fireToast('Importing recipe… 12 ingredients detected')}>Import</Button>
      </div>
      <div style={{ marginTop: 18, padding: 16, background: 'var(--paper-warm)',
                      border: '1px solid var(--line)', borderRadius: 12, display: 'flex', gap: 16 }}>
        ...
        {methods.map(m => (
          <Card key={m.kind} pad={0} hover
            ...
            <div style={{ padding: 18, flex: 1, display: 'flex', flexDirection: 'column' }}>
              <div className="serif" style={{ fontSize: 19, letterSpacing: -0.2, color: 'var(--ink)' }}>{m.title}</div>
              <p style={{ margin: '6px 0 16px', fontSize: 12.5, color: 'var(--ink-muted)', lineHeight: 1.5 }}>{m.desc}</p>
              <div style={{ marginTop: 'auto' }}>
                <Button full variant="primary" iconRight="arrowRight"
                        onClick={(e) => { e.stopPropagation(); fireToast('Capture opens in a later update'); }}>Open</Button>
              </div>
            </div>
          </Card>
        ))}
...
export function Settings() {
  ...
  return (
    <div style={{ padding: '28px 28px 56px', display: 'grid', gridTemplateColumns: '240px 1fr', gap: 22 }}>
      <aside>
        {['Profile', 'Preferences', 'Equipment', 'Notifications', 'Household', 'Connections', 'Privacy'].map(s => {
          ...
          return (
            <button key={s} onClick={() => setSection(s)} style={{
              ...
              boxShadow: active ? 'var(--sh-1)' : 'none', cursor: 'pointer', fontFamily: 'inherit',
            }}>{s}</button>
          );
        })}
      </aside>
      <div style={{ display: 'grid', gap: 18 }}>
        <Card pad={26}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
            ...
            <Button variant="secondary" icon="edit" onClick={() => fireToast('Editing your profile')}>Edit profile</Button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
            {[
              ...
              <div key={e.l} style={{
                ...
                <div style={{ fontSize: 9.5, marginTop: 3, opacity: 0.7 }}>{e.on ? 'Available' : 'Not yet'}</div>
              </div>
            ))}
          </div>
        </Card>
